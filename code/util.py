...
def evaluate(idx2pos, evaluation_dataloader, model, criterion, attn_loss_criterion, using_GPU):
    """
    ...
    for (eval_pos_seqs, eval_text, eval_lengths, eval_labels, eval_para_idx,
        ...
        with torch.no_grad():
            ...
            eval_para_idx = Variable(eval_para_idx)
            eval_paraset = Variable(eval_paraset)
            eval_paraset_attn = Variable(eval_paraset_attn)
            eval_paraset_len_mask = Variable(eval_paraset_len_mask)

        if using_GPU:
            eval_text = eval_text.cuda()
            eval_lengths = eval_lengths.cuda()
            eval_labels = eval_labels.cuda()
            eval_para_idx = eval_para_idx.cuda()
            eval_paraset = eval_paraset.cuda()
            eval_paraset_attn = eval_paraset_attn.cuda()
            eval_paraset_len_mask = eval_paraset_len_mask.cuda()
        ...
        attn_loss = attn_loss_criterion(attn_weight, eval_para_idx, eval_labels)
        ...
        if len(tmp_arr) != 0:
            tmp_arr = [0] * (len(eval_pos_seqs) - len(tmp_arr)) + tmp_arr
        ...
        total_eval_loss += (attn_loss.item())
...
def print_info_class(grid):
    ...
    print('PRFA performance  ', precision, recall, accuracy, f1)
    ...
    def collate_fn(batch):
        ...
        for pos, __, __, __, __, paraset in batch:
            if len(pos) > max_length:
                max_length = len(pos)
            for p in paraset:
                if p.shape[1] > max_para_len:
                    max_para_len = p.shape[1]
        ...
        for pos, text, length, label, para, paraset in batch:
            ...
            for para_candidate in paraset:
                ...
                para_candidate = para_candidate[2]
                can_para_len = para_candidate.shape[0]
                amount_to_pad = max_para_len - can_para_len
                pad_para = torch.zeros(amount_to_pad, para_candidate.shape[1])
                para_candidate = torch.Tensor(para_candidate)
            ...
            pad_para_tensor = torch.zeros(amount_to_pad,padded_para_set[0].shape[0],padded_para_set[0].shape[1])
...
def elmo_paraphrase_embed(paraphrase, paraphrase_index_dict, elmo_embddings):
    '''Embeded the paraphrase with elmo embeddings

    :param paraphrase: the string of the paraphrases
    :param paraphrase_index_dict: the index for the paraphrase, used as a key to acquire the elmo embeddings
    :param elmo_embddings: the elmo embeddings matrix
    :return: the elmo embedding for the paraphrase
    '''
    index = paraphrase_index_dict[paraphrase]
    keys = elmo_embddings.keys()
    tmp = elmo_embddings[index]
    return elmo_embddings[index]
